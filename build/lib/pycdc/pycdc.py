...
Usage:
    ...
    2. Run the code to get the pandas dataframe with CDC.
...
class CDCGenerator:
    ...
    def __init__(self, keyColumn,sourceDataframe,previousDataframe) -> None:
        ...
        if self._sourceDataframe.empty or self._previousDataframe.empty:
            raise EmptyDataframeError('Empty Dataframe.')
        ...
        try:
            if len(self._keyColumn)!=0:
                ...
                if len(missingColumns)!=0:
                    raise ColumnNotFoundError(f'One or more key columns not found in the source DataFrame. {missingColumns}')
            else:
                raise EmptyColumnListError('Empty columns list')
        except Exception as e:
            raise EmptyColumnListError(f"{e}")
    ...
    def _update_cdc(self, row, previousDF):
        ...
        try:
            ...
            # Check if any row in previousDF matches the conditions
            for _, prev_row in previousDF.iterrows():
                if all(current_product_id == prev_row[self._keyColumn]) and current_hash_value != prev_row['HashValue']:
